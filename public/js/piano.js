app.controller('Piano', function ($scope, $http, $timeout)
{
    $scope.listId = 1;
    $scope.songList = [];
    $scope.favorites = [];
    ...
    $scope.selectedItem = function(item, index)
    {
        if( typeof item !== 'undefined' ){
            $http({ method: "POST", url: "/ajax", data: {command: 'add_to_piano_favorites', id: item.originalObject.ID } }).then(
                function success(respond){
    ...
    {
        $http({ method: "POST", url: "/ajax", data: {command: 'get_piano_favorites' } }).then(
            function success(respond){
    ...
    $scope.clearFavorites = function(){
        if($scope.favorites.length > 0)
            $scope.confirmationDialog("Список выбранных песен", function() {
                $http({method: "POST", url: "/ajax", data: {command: 'clear_piano_favorites'}}).then(
                    function success(respond) {
                        $scope.reloadFavorites();
                    },
                );
    ...
    $scope.deleteFavoriteItem = function(fav_id, fav_title){
        $scope.confirmationDialog(fav_title, function(){
            $http({ method: "POST", url: "/ajax", data: {command: 'delete_piano_favorite_item', id: fav_id } }).then(
                function success(respond){
                    $scope.reloadFavorites();
                },
            );
    ...
    $scope.addSongToFavorites = function( songId ){
        ...
        $http({ method: "POST", url: "/ajax", data: {command: 'add_to_piano_favorites', id: songId } }).then(
            function success(respond){
                $scope.reloadFavorites();
            },
            function error(erespond){
