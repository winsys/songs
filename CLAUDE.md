...
- The rest of the non-fullscreen pages use the same tokens: `login.html` / `index.html` have their whole `<style>` rewritten (clarity over density: one centred card; the hub is a responsive tile grid — the card itself is the CSS grid, labels span all columns); `settings.html`, `import.html` and `observer.html` got the sticky `.cx-bar` plus a "COMPACT UI SKIN" block appended LAST in their `<style>` (markup otherwise unchanged; observer's dark viewers untouched). `musician.html` and the two screens are fullscreen and stay as they are.
