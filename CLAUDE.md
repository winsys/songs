...
- `Ajax_Observer` — observer page: `observer_get_state`, `observer_list_messages`; leader-side `observer_set_active` / `observer_set_song` / `observer_set_text` / `observer_join_link` (QR of the group's observer login, read-only counterpart of `get_join_link`); the `$observerCommands` whitelist enforced in `Ajax::execute` for the observer role
...
**Observer channel (Aug 2026):** table `current_observer` (one row per group: `active`, `song_id`, `verse_idx`, `langs`, `text`, `title`) + group-scoped WS event `observer_update` carrying the state (without the song payload). Written ONLY by `observer_set_active` / `observer_set_song` / `observer_set_text` from the leader page and the tech console (sent in addition to — never instead of — `set_image` / `set_tech_image` / `set_text` / `set_leader_text` / `set_bible_text` / `set_message_text` / `clear_image`; no-ops while the group's toggle is off; the toggle is shared by both pages). Two layers: the song (observers pick language / image type) and the text overlay (tech's Bible verse / message paragraph, shown with its caption until cleared; a song command clears it). Read by `/observer` (`observer_get_state` returns the song with all `TEXT*` + image groups). Fully separate from `current` and `current_notes`; screens and musicians never touch it.

**Sheet-music image groups (Aug 2026):** every collection has ordered image groups (`song_image_groups`, defaults «НОТЫ» IS_MAIN + «АККОРДЫ», names translatable per UI language in `NAMES` JSON — render via `SongImages::displayName()`); a song has AT MOST ONE image per group. Storage is file-based (`app/SongImages.php`): the main group's image is the legacy derived `/images/<L>/<NUM>.jpg`, every other group's is `/images/<L>/g<GROUP_ID>/<NUM>.jpg|png`. The musician page asks `get_notes` with `with_groups:1` (tech console keeps the legacy one-field shape) and remembers the chosen group in `sessionStorage`. ZIP import (`import_song_images_zip`, `group_id` + `mode` replace|add) runs on the pure-PHP `app/ZipReader.php` — production has no zip extension. Song numbers may contain Cyrillic/spaces (`д001`, `304 (1)`); never ASCII-filter them. Song rows (`get_favorites`, `get_favorites_with_text`, `piano_get_favorites`, `get_song_list`, `get_songs_for_search`) carry `imageSrc` = the main sheet path + `?v=<mtime>` (`?v=0` when there is no file; `SongImages::withImageSrc`) — list / search thumbnails, previews and the leader's fullscreen notes use it, because a replaced sheet keeps its file name and browsers show the cached old one; `imageName` (bare path) stays the song's identity (notes channel, comparisons). Replacing/deleting the MAIN group's image fires `update_needed`.

**Parallel Bible display (Sept 2026):** the tech console's Bible mode has a primary translation (row-2 buttons; `bibleLang` = its LANG) plus optional parallel languages («Параллельные языки», each mapped to one translation of that language, persisted in localStorage). `get_bible_parallel` (Ajax_Tech) returns the chapter texts keyed by the PRIMARY verse number, mapped through `app/BibleMap.php`: Synodal-LXX ↔ Masoretic Psalm chapters, Hebrew Joel/Malachi splits, psalm-superscription offsets via segment end-alignment. Numbering tradition is detected from the data (Ps 9 max verse > 30 ⇒ LXX; Joel 4 chapters / Malachi 3 ⇒ Hebrew), per-request cache, no schema flags. All other books join verse-number-to-verse-number. The display pipeline is untouched: the console composes flat text (language blocks joined with `- - - - - - - -`) and sends it via the existing `set_bible_text` / `observer_set_text`. NB: «Новый Русский Перевод» (tr 5) has known-corrupt data (missing 4 Kings, duplicated verses in 3 Kings, shifted Psalms) — needs a re-import.
...
### Compact UI (Sept 2026)
- Spec: `docs/ui-redesign-tech-leader.md`. Shared tokens and chrome components (application bar, context bar, segmented switch, chips, panes) live in the namespaced `public/css/compact.css` (`--cx-*` / `.cx-*`, linked from `layout.html`, bump its `?v=`). Redesigned so far: `leader.html` (phone <= 540px, tablet 541-899px, >= 900px rail + list; the sheet-music thumbnail is ALWAYS the rightmost element of a song row; the row actions ✏️ / ¶ / Аа are exactly the thumbnail's size and set wider apart — one set of variables `--ld-act-w/-h/-space` on `.favorites-list`, change them together; songs are edited / added with the tech console's dialog — ✏️ before ¶ on desktop only (`min-width: 900px` + `hover: hover` + `pointer: fine`; tablets, also in the >= 900px layout, don't show it), «Изменить» in the swipe tray on every size, «＋ Добавить» next to «Все песни» creates the song in the selected collection), `tech.html` (desktop only, the page never scrolls: bars + independently scrolling panes; scroller ids `techPlaylist`, `bible-*-panel`, `messages-*-panel` are used by `tech.js`), `sermon_prep.html` (desktop only; progressive Bible navigation via the view-only `nav.up`; the books level is two columns, Old Testament | New Testament by canonical `BOOK_NUM` 1-39 / 40-66 — `getTestamentBooks(nt)`, same split as the observer page). Each template keeps its page CSS inline; the compact block is appended LAST in the `<style>` on purpose. `piano.html` mirrors the leader layout and the swipe-to-delete (no drag handle, no ¶, no broadcast, no song editing — the tray holds «Удалить» only; `deleteFavoriteNow` → `piano_delete_favorite`) — its head CSS is a trimmed copy of the leader one, keep them in sync by hand.

- Home / Observer / Settings / Data Import follow `docs/ui-redesign-main-observer-settings-import.md`. All four use VIEW-ONLY template state set by `ng-init` (no controller changes): `index.html` = one bar (🎵 Worship Songs + user) and grouped destination rows (the list itself is a 2-column grid, 1 on phones); `observer.html` = library rail (modes + filters of the active mode) + result pane on >= 700px, fixed bottom tab bar + filter sheet (`obsUi.filters`) on phones, dark viewers untouched; `settings.html` = category rail + one form surface, every card carries `setUi.cat === '<cat>'` in its `ng-show` (keep it when adding a card!), phone drill-down `setUi.drill`, fixed save strip; `import.html` = task rail | form | help-and-protocol aside (`impUi.songs` / `impUi.langs`, messages reuse `msgSubTab`; the help blocks and the three logs were MOVED into `.imp-aside`, ids `songLogEl` / `msgLogEl` / `langLogEl` kept), the aside is a right drawer below 1200px (`impUi.drawer`). Form content: settings fields are one alignment grid (`.set-surface .settings-field` = label column + control column, colour swatch + hex as one control, ranges stay sliders with the value in the label, `.set-preview` = live preview of the main / streaming display values); import forms are a two-column grid too (`.imp-main .import-section`, a field group inside an ng-show wrapper needs class `imp-fields`), primary action at the end of the form on the right. `login.html` has its `<style>` rewritten on the tokens. `musician.html` and the two screens are fullscreen and stay as they are.
- Chip groups that may outgrow their bar (tech: collections, Bible translations, parallel languages) are wrapped in `.cx-scroller` + `[data-cx-scroll]`; `public/js/cx_scroll.js` shows edge arrows only on overflow, maps the mouse wheel to sideways scrolling and keeps `.btn-selected` in view. Neighbour groups are `flex-shrink:0`, so they are never pushed off the screen.
- Search fields with a clear button use `<span class="cx-clearable"><input><button class="cx-clear">` (compact.css); the tech song search tracks its text through angucomplete `input-changed` (`songSearchText` / `clearSongSearch`).
- Tech console, Bible mode: the context bar has a SECOND row (`.tc-ctx-parallel`) for the parallel languages — in one row they were pushed off-screen by a long translation list.
- Sermon prep source panel (544px): book filter + Bible full-text search (`search_bible_verses`) and message quick paragraph results (`search_message_paragraphs`, PARA_IDX = position among non-empty paragraphs) — same modes as the tech console. The verses panel title is ALWAYS rendered: a title appearing on the first click shifts the list and kills the double-click insertion.
- Two-language Bible chips (sermon prep «Второй язык чипа», `bibleSecond` in localStorage `prep_bible_second`): second text via `get_bible_parallel`; the chip carries `data-display-text` / `data-display-title` (language blocks joined with `- - - - - - - -`), and `sermon.js` shows that ready-made text through `set_message_text` instead of re-fetching one verse. Single-language chips are unchanged.
...
- **Persistent login (`app/RememberMe.php`, Sept 2026 — same pattern as pokupki):** every successful login (password, Google, observer join link) issues a 1-year `remember` cookie (`selector:validator`, validator stored as sha256, rotated on every auto-login); `App::run()` falls back to it when the PHP session is gone, `/logout` revokes the current device's token. Table `auth_token` auto-creates (and upgrades itself) on first use. Rotation is race-safe (Sept 2026): the previous hash stays in `prev_validator_hash` and is accepted for `GRACE` (600 s) after `rotated_at`, so a page load and an ajax poll arriving together without a session no longer revoke the token (manual DDL `database/migrations/add_auth_token_rotation_grace.sql`); `issue()` deletes the row of the cookie the device already held. Every outcome goes to the Apache error log with prefix `RememberMe:` (ip, path, xhr/sess flags, short UA), including `no cookie` on page loads — read that first when a device "forgets" its login. Together with the PWA manifest (`public/manifest.json` + icons 192/512, standalone display) the home-screen icon opens as an app, already logged in after the first login on that device.
...
- Observer join link (Aug 2026): `/join/<token>` → `Security::joinByToken()` — `users.JOIN_TOKEN` (observer accounts only, issued/regenerated by `Ajax_Settings::get_join_link`; the leader page reads it via `Ajax_Observer::observer_join_link` while its broadcast toggle is on — the «📱 QR-код» button next to the toggle) starts the session like a password login and lands on `/observer`. QR rendered client-side by the vendored `public/js/vendor/qrcode.min.js` (qrcode-generator, MIT).
...
- Layout per `docs/ui-redesign-sermon-display.md` (Sept 2026): landscape-only, always fills the viewport (`100dvh`), CSS grid with `--sd-notes` = 55% (preview 45% at every tablet width — Pavel's decision, overrides the spec's 58/42); phone landscape (`max-height: 540px`) = notes 62% | 50px control dock `#sermon-dock` (Back, A−, A+, Clear; Fullscreen pinned to the bottom behind a divider) | preview (a 45% preview was tried on 2026-09-25 and reverted by Pavel). `#notes-body` never scrolls sideways: `overflow-x: hidden` + `overflow-wrap: anywhere`, editor media / tables capped at `max-width: 100%`, the chip reference may wrap. Tablet toolbar lives in the notes pane only (Back, title/date, A−/A+). The sermon title («Выберите проповедь» / the sermon's name + ▾) IS the selector on every size: the ONE `#sermon-select` lies invisibly (opacity 0) over `#notes-title-wrap`, so a tap opens the native picker — no dropdown / ☰ menu any more; fullscreen is a quiet overlay in the preview's lower-right corner. `clearDisplay()` is a thin wrapper over the existing toggle-off / `stopVideo()` paths. Portrait + touch shows a rotate request. The overrides are appended LAST in the page `<style>`; notes colours still come from the runtime `--sn-*` variables.
...
app/
  ...
  BibleMap.php               # versification mapping between translations (parallel Bible display)
...
public/js/
  ...
  song_editor.js             # song edit / add dialog logic (tech + leader), window.installSongEditor
...
templates/
  ...
  song_edit_dialog.html      # song edit / add dialog partial (PHP-included by tech.html and leader.html)
  sermon.html
