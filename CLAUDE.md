...
- Observer join link (Aug 2026): `/join/<token>` → `Security::joinByToken()` — `users.JOIN_TOKEN` (observer accounts only, issued/regenerated by `Ajax_Settings::get_join_link`) starts the session like a password login and lands on `/observer`. QR rendered client-side by the vendored `public/js/vendor/qrcode.min.js` (qrcode-generator, MIT).
