...
- Layout per `docs/ui-redesign-sermon-display.md` (Sept 2026): landscape-only, always fills the viewport (`100dvh`), CSS grid with `--sd-notes` = 55% (preview 45% at every tablet width — Pavel's decision, overrides the spec's 58/42); phone landscape (`max-height: 540px`) = notes (rest) | 50px control dock `#sermon-dock` (Back, A−, A+, Clear; Fullscreen pinned to the bottom behind a divider) | preview 45% (Pavel 2026-09-25, overrides the spec's 62% notes). Tablet toolbar lives in the notes pane only (Back, title/date, A−/A+). The sermon title («Выберите проповедь» / the sermon's name + ▾) IS the selector on every size: the ONE `#sermon-select` lies invisibly (opacity 0) over `#notes-title-wrap`, so a tap opens the native picker — no dropdown / ☰ menu any more; fullscreen is a quiet overlay in the preview's lower-right corner. `clearDisplay()` is a thin wrapper over the existing toggle-off / `stopVideo()` paths. Portrait + touch shows a rotate request. The overrides are appended LAST in the page `<style>`; notes colours still come from the runtime `--sn-*` variables.
