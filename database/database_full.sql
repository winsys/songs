...
CREATE TABLE IF NOT EXISTS `bible_verses` (
  ...
  FULLTEXT KEY `ft_bible_text` (`TEXT`,`TEXT_LT`,`TEXT_EN`),
  CONSTRAINT `fk_bible_verses_book` FOREIGN KEY (`BOOK_ID`) REFERENCES `bible_books` (`ID`) ON DELETE CASCADE
) ENGINE=InnoDB AUTO_INCREMENT=93423 DEFAULT CHARSET=utf8;
...
CREATE TABLE IF NOT EXISTS `languages` (
  `code` varchar(10) COLLATE utf8mb4_unicode_ci NOT NULL COMMENT 'Language code: ru, lt, en, de …',
  `label` varchar(16) COLLATE utf8mb4_unicode_ci NOT NULL COMMENT 'Button label: RU, LT, EN …',
  `col_suffix` varchar(16) COLLATE utf8mb4_unicode_ci NOT NULL COMMENT 'DB column suffix: empty → TEXT, _LT → TEXT_LT …',
  `sort_order` int(11) NOT NULL DEFAULT '0' COMMENT 'Display order',
  `is_default` tinyint(1) NOT NULL DEFAULT '0' COMMENT '1 = default language (fallback)',
  PRIMARY KEY (`code`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='Available languages. Add/remove a language only in this table.';
...
CREATE TABLE IF NOT EXISTS `messages` (
  ...
  `TEXT_PL` longtext COLLATE utf8mb4_unicode_ci,
  PRIMARY KEY (`ID`),
  KEY `idx_messages_code` (`CODE`),
  FULLTEXT KEY `ft_messages` (`TITLE`,`TEXT`)
...
CREATE TABLE IF NOT EXISTS `song_list` (
  ...
  `TEXT_PL` longtext,
  PRIMARY KEY (`ID`),
  KEY `idx_song_list_listid` (`LISTID`),
  KEY `idx_song_list_name` (`NAME`)
...
CREATE TABLE IF NOT EXISTS `standard_wallpapers` (
  ...
  PRIMARY KEY (`id`),
  UNIQUE KEY `src` (`src`)
) ENGINE=InnoDB AUTO_INCREMENT=7 DEFAULT CHARSET=utf8mb4 COMMENT='Standard wallpapers available to all users';
...
) ENGINE=InnoDB AUTO_INCREMENT=10 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='Stores multiple Google accounts linked to user logins';
...
CREATE TABLE IF NOT EXISTS `user_settings` (
  ...
  `sermon_notes_font_size` smallint(6) NOT NULL DEFAULT '100',
  ...
  `ui_lang` varchar(5) NOT NULL DEFAULT 'ru' COMMENT 'UI language code (ru/de/en/lt/pl). Independent from content languages.',
...
-- Dumping structure for table songs.signup_requests
